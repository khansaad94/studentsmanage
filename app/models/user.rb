class User < ActiveRecord::Base
  ...
  scope :search_celeb_by_name, ->(text) { joins(:profile).where('is_approved =? AND (first_name like ? OR last_name like ?)', true, "%#{text}%", "%#{text}%")}
  # scope :search_celeb_by_name, ->(text) { joins(:profile).where('user_type =? AND is_deleted =? AND is_active =? AND (first_name ilike ? OR last_name ilike ? OR full_name ilike ?) AND celebrities.verified_account=? AND away_mode =?', 'celebrity', false, true, "%#{text}%", "%#{text}%","%#{text}%", true, false) }
